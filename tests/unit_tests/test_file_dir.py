...
def test_create_dir_when_dir_exist(tmp_path, capsys):
    '''Main path test to assert if custom create dir function is not generating directories if they exist and do not fail if exist'''
    d = tmp_path / "existing_dir"
    d.mkdir()
    result = m_dir.create_dir_if_not_exist(d)
...
def test_create_dir_when_dir_not_exist(tmp_path, capsys):
    '''Main path test to assert if custom create dir function is generating directories if they do not exist and do not fail if exist'''
    d = tmp_path / "new_dir"
...
def test_create_dir_when_user_has_no_edit_permission(tmp_path):
    '''Alternate path test to assert if custom create dir function is failing if direcotry do not exist but no permission to make it'''
    ...
    with pytest.raises(PermissionError) as exc_info:
        m_dir.create_dir_if_not_exist(d)
...
def test_clean_up_folder_starting_with(tmp_path, capsys):
    '''Main path test to assert if folder cleanup based on prefix works'''
    d = tmp_path / "dir_to_be_deleted"
    d.mkdir()
    assert os.path.exists(d)
    result = m_dir.clean_up_folder_starting_with(tmp_path, "dir")
    out, err = capsys.readouterr()
    assert not os.path.exists(d)
    assert result == True
    assert err == ''
...
def test_clean_up_folder_starting_with_when_not_empty(tmp_path, capsys):
    '''Main path test to assert if folder cleanup based on prefix works, when directory not empty'''
    d = tmp_path / "dir_to_be_deleted"
    d.mkdir()
    os.chdir(d)
...
def test_clean_up_folder_starting_with_when_bad_prefix(tmp_path, capsys):
    '''Alternate path test to assert if folder cleanup based on prefix works'''
    d = tmp_path / "dir_to_be_deleted"
    d.mkdir()
    assert os.path.exists(d)
    result = m_dir.clean_up_folder_starting_with(tmp_path, "bad_dir")
    out, err = capsys.readouterr()
    assert os.path.exists(d)
    assert result == True
...
def test_clean_up_folder_starting_with_when_name_equals_prefix(tmp_path, capsys):
    ...
    d.mkdir()
    assert os.path.exists(d)
    result = m_dir.clean_up_folder_starting_with(tmp_path, "dir_to_be_deleted")
    out, err = capsys.readouterr()
    assert not os.path.exists(d)
    assert result == True
    assert err == ''